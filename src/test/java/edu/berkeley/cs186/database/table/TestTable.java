...
public class TestTable {
  ...
  public void testTableSimpleInsert() throws DatabaseException {
    ...
    assertEquals(0, rid.getSlotNumber());
  ...
  public void testTableMultiplePages() throws DatabaseException {
    ...
    for (int i = 0; i < numEntriesPerPage; i++) {
      ...
      assertEquals(1, rid.getPageNum());
      assertEquals(i, rid.getSlotNumber());
    }
  ...
  public void testRecordUpdate() throws DatabaseException {
    ...
    dataValues.add(new BoolDataType(false));
    dataValues.add(new IntDataType(2));
    dataValues.add(new StringDataType("abcdf", 5));
    dataValues.add(new FloatDataType((float) 1.5));
    Record updated = new Record(dataValues);

    table.updateRecord(dataValues, rid);
    Record output = table.getRecord(rid);
  ...
  @Test
  public void testRandomRecordFetch() throws DatabaseException {
    Record input = TestUtils.createRecordWithAllTypes();
    ...
    Record random = table.getRecord(new RecordID(1, 59));
    assertEquals(input, random);
  }
  ...
  public void testTableDurablePartialAppend() throws Exception {
    ...
    assertEquals(2, rid.getPageNum());
    assertEquals(1, rid.getSlotNumber());
  }
